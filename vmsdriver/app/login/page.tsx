...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user?.role !== "Driver") {
        setError("Unauthorized role. Please log in with a Driver account.");
        removeTokenCookie();
        setLoading(false);
        return;
      }
      ...
      <Card className="w-full max-w-md shadow-2xl border-0 backdrop-blur-sm bg-white/95 relative z-10">
        <CardHeader className="space-y-3 pb-8">
          ...
          <CardDescription className="text-center text-base text-gray-600">
            Vehicle Management System - Driver Access
          </CardDescription>
        </CardHeader>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="space-y-2">
              ...
              <Input
                ...
                required
                disabled={loading}
                className="h-12 text-base transition-all focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white text-gray-900"
              />
            </div>
            <div className="space-y-2">
              <label
